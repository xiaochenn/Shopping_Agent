...
def get_existed_goals(all_products, product_prices, if_persona=False):
    goals = []
    budget_semantics = load_budget_semantics()
    cnt_atts = defaultdict(int)
    cnt_1, cnt_2, cnt_3 = 0, 0, 0
    goal_instructions = []
    for task_id, item in enumerate(all_products):
        ...
        if len(item['instructions']) != 1:
            raise ValueError(
                f"task {task_id}: Environment v2.1 requires exactly one instruction"
            )
        for product in item['instructions']:
            ...
            budget_constraint = constraint_for_task(task_id, budget_semantics)
            validate_constraint_source(
                budget_constraint,
                asin=asin,
...
def get_reward(purchased_product, goal, price, options, **kwargs):
    ...
    price_lower = goal.get('price_lower')
    price_upper = goal.get('price_upper')
    r_price = int(
        (price_lower is None or price >= price_lower)
        and (price_upper is None or price <= price_upper)
    )
